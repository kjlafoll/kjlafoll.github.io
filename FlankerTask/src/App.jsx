...
//Flanker Task- Faces Screen 2
const FacesScreen2 = ({ onButtonClick, value, onChange }) => (
  <Box
    sx={{
      display: 'flex',
      justifyContent: 'center',
...
//Rotate Screens
  function App() {
    ...
    //creates state variable to store all inputs
    const [inputValue, setInputValue] = useState('');
    //function handleChange updates inputValue variable when onChange event triggered
    const [targetScreen, setTargetScreen] = useState(8);
   
    const handleChange = (event) => {
      const response = event.target.value.trim().toUpperCase(); // Get the value from the input field
      setInputValue(response); // Update input value
      if (screen >= 9 && (response === 'Q' || response === 'P')) {
        console.log('handleChange:', response); 
        saveResponse(response); // Save the response to local storage
      }
    };    

    const saveResponse = (response) => {
      console.log('saveResponse:', response);
      // Load existing responses from localStorage web API or initialize empty array
    ...
    useEffect(() => {
      const interval = setInterval(() => {
        if ((screen == 9 || screen == 10 || screen == 11) && inputValue === '') {
          return;
        }
        if(screen == 6 || screen == 8) {
          setDelay(1000); // Set the delay to 1 second
          if(screen == 8) {
        ...
        else if((screen == 9 || screen == 10 || screen == 11) && inputValue != '') {
          setInputValue('');
          setDelay(2000); 
        }
      }, delay);
    ...
    const switchScreen = (nextScreen) => {
      // Handle button click and switch screens
    ...
    return (
      <div>
        ...
        {screen === 11 && <FacesScreen2 value={inputValue} onChange={handleChange}/>}
      </div>
    );
  }
