...
const csvFilePath = 'test_battery2.csv'; // must be accessible where you host the task
...
// Make this synchronous (your old version was async but used like a boolean)
...
// ---------- Frontloaded overlays (promisified) ----------
function showLLMInstructionsSlide() {
  return new Promise(resolve => {
    ...
    // Use a one-time handler to avoid stacking listeners
    const handler = () => {
...
function askOneToMQuestion(qObj) {
  return new Promise(resolve => {
    ...
    textEl.textContent = qObj.question;
    input.value = '';
    overlay.style.display = "flex";
...
async function runFrontloadedToMBattery() {
  ...
  player1Data.push({
    tom_questions: selectedQuestions.map(x => ({ inventory: x.inventory, question: x.question })),
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  startButton.addEventListener("click", async function () {
    ...
    // (2) LLM placeholder slide
    await showLLMInstructionsSlide();

    // (3) Frontloaded ToM battery (one question from each inventory)
...
async function setupGame() {
  ...
  // Use index-based progress since Trial numbers may not be contiguous after filtering
  ...
  document.getElementById("status").textContent = `Trial ${currentGameIndex + 1}: Player 1, make your move!`;
    ...
    if (currentCell === "D") {
      ...
      setTimeout(() => {
        ...
        if (computerAction === "stay") {
          ...
          // NOTE: your original code always added p1Payoff[1] here; leaving as-is
